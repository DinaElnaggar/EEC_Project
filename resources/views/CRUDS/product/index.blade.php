                    ...
                    <!--end::Table head-->
                        <tbody>
                            @foreach($products as $index => $data)
                                <tr>
                                    <td>{{$index +1}}</td>
                                    <td> <a data-fancybox="" href="{{get_file($data->image)}}">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg mw-650px">
            ...
            <div class="modal-content" id="modalContent">
                ...
                <div class="modal-header">
                    <!--begin::Modal title-->
                    <h2><span id="operationType"></span> Product </h2>
                    <!--end::Modal title-->
                    <!--begin::Close-->
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#addBtn').on('click', function () {
                $.ajax({
                    url: "{{ route('products.create') }}", // Route to fetch the create form
                    type: 'GET',
...
<script>
    $(document).on('click', '.editBtn', function () {
        let id = $(this).data('id');
        $.ajax({
            url: `/products/${id}/edit`, // Your edit route
            type: 'GET',
            success: function (response) {
    ...
    <script>
        $(document).on('click', '.deleteBtn', function () {
            ...
            if (confirm('Are you sure you want to delete this product?')) {
                $.ajax({
                    url: `/products/${id}`, // Your delete route
                    type: 'DELETE',
                    data: {
